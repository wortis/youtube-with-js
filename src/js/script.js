const switcer = document.querySelector('#cbx'),
...
bindSlideToggle('.hambutger', '[data-slide="nav"]', '.header__menu', 'slide-active');
...
let night = false;
switcer.addEventListener('change', () => {
    switchMode();
});
 ...
 //   for (let i = 0; i < data[0].length; i++) {
  ...
  //      card.innerHTML = `
   ...
   //                     <div class="videos__item-descr">
     ...
     //   }, 10);
     //   if(night === true) {
     //       card.querySelector('.videos__item-descr').style.color = '#000';
    //        card.querySelector('.videos__item-views').style.color = '#000';
     //   }
...
function start() {
    gapi.client.init({
        'apiKey': 'AIzaSyBJvStPg8RwewKEqPhx8CxknUaOBdzV4zc',
...
modal.addEventListener('click', (e) => {
    if (!e.target.classList.contains('modal__doby')) {
        closeModal();
    }
    document.addEventListener('keydown', function(ent) {if (EventTarget.keyCode === 27) {closeModal();}});
});
